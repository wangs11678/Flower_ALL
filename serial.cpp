...
int UART0_Open(int fd, char* port)
{
    ...
    if(FALSE == fd)
    {
        perror("Serial: Can't open serial port");
        return FALSE;
    }
    //串口是否为阻塞状态
    ...
    {
        printf("Serial: Standard input is not a terminal device\n");
        return FALSE;
    }
    return fd;
}
...
int UART0_Set(int fd, int speed, int flow_ctrl, int databits, int stopbits, int parity)
{
    ...
    */
    ...
    case 8:
        ...
        break;
    default:
        fprintf(stderr, "Serial: Unsupported data size\n");
        return FALSE;
    }
    //设置校验位
    switch (parity)
    ...
    case 'S': //设置为空格
        ...
        break;
    default:
        fprintf(stderr,"Serial: Unsupported parity\n");
        return FALSE;
    }
    // 设置停止位
    switch(stopbits)
    ...
    case 2:
        ...
        break;
    default:
        fprintf(stderr,"Serial: Unsupported stop bits\n");
        return FALSE;
    }
...
* 出口参数： int
*******************************************************************/
int serial_send(char *serialPort, unsigned char* cmd)
{
    int fd; //文件描述符
    int err; //返回调用函数的状态
    int len;
    ...
    do
    {
        ...
        //printf("Serial: Set port exactly!\n");
    }
    while(FALSE == err || FALSE == fd);
      
    len = UART0_Send(fd, cmd, 1);
    if(len < 0)
        printf("Serial: Send data failed!\n"); 
  	else
  	{
  		UART0_Close(fd);
  		return TRUE;
